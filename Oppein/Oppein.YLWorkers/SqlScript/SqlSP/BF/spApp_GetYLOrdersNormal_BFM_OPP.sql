alter PROC [dbo].[spApp_GetYLOrdersNormal_BFM_OPP]
	@ordID INT
	,@pageName NVARCHAR(50)=N'包覆门'
...
BEGIN
	...
	JOIN dbo.ItemItemFamilies itmItemF (NOLOCK) ON itmItemF.itmID = t.itmID AND itmItemF.itmfCode =N'BFMKM_MB'
	OUTER APPLY
	(
		SELECT 
	...
	outer apply(
		SELECT
			...
			,MAX(case when ibv.atbID=115  then ibv.itmavValue end) ainfo1   --拉手位置备注
			,MAX(CASE WHEN ibv.atbID=45 THEN ibv.itmavValue END) AS info3	--开门方向(CAD)
			--,MAX(case when ibv.atbID=362  then ibv.itmavValue end) wenlin   --纹理
		from dbo.ItemAttributeValues ibv(NOLOCK)
		WHERE ibv.itmID = t.itmID 
			AND ibv.atbID IN(362,44,45,115)
	)itmAv
	WHERE t.dlCode=N'MA'
...
BEGIN
	...
	INSERT INTO [dbo].[CUS_YLOrdOriginalBOMData_OPP](ordId,pageName,itmID,itmIDInstance,olnID,olnIDInstance,itmIDParent,itmIDParentInstance,itmIDSA,itmDescriptionSA,dlCode,oriReqQty,primaryUOMCode,itmItemNumber,itmDescription,itmfCode,topSurCode,corCode,dimCX,dimCY,dimCZ,dimFX,dimFY,dimFZ,info3,info6,info15)
	SELECT 
		@ordID AS ordID
		...
		,CAST(oDim.dimFZ AS FLOAT) AS dimFZ
		,itmAv.info3	--开门方向（CAD)
		,itmAv.info1 AS info6	--备注
		,itmAv.ainfo1 AS info15
	FROM T t
	JOIN T SA ON t.olnID=SA.olnID AND SA.dlCode=N'SA'
	JOIN dbo.ItemItemFamilies itmItemF (NOLOCK) ON itmItemF.itmID = t.itmID AND itmItemF.itmfCode =N'BFMKM_MB'
	OUTER APPLY
