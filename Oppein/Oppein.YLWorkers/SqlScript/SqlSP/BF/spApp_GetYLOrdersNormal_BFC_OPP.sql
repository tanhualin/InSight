ALTER PROC [dbo].[spApp_GetYLOrdersNormal_BFC_OPP]
	@ordID INT
	,@pageName NVARCHAR(50)=N'包覆抽面'
...
BEGIN
	...
	(
		...
		JOIN Product.Products pd ON pd.pdID = olnp.pdID
	...
	SELECT 
		...
		,org.actions
		,ha.Remark AS info6	--备注
		,ISNULL(ha.newdmRemark,itmAv.ainfo1) AS info14
		,ISNULL(ha.holeRemark,itmAv.info1) AS info15
		,ISNULL(org.info6,ha.Remark) AS oInfo6
		,ISNULL(org.info14,ha.newdmRemark) AS oInfo14
		,COALESCE(org.info15,ha.holeRemark,itmAv.info1) AS oInfo15
	...
	JOIN dbo.ItemItemFamilies itmItemF (NOLOCK) ON itmItemF.itmID = t.itmID AND itmItemF.itmfCode =N'BFMKM_CM'
	OUTER APPLY
	(
		SELECT 
	...
	outer apply(
		SELECT
			...
			,MAX(CASE WHEN ibv.atbID=45 THEN ibv.itmavValue END) AS info2	--开门方向(CAD)
			,MAX(CASE WHEN ibv.atbID=115 THEN ibv.itmavValue END) AS ainfo1	--拉手孔备注
		from dbo.ItemAttributeValues ibv(NOLOCK)
		WHERE ibv.itmID = t.itmID 
			AND ibv.atbID IN(44,45,115)
	)itmAv
...
BEGIN
	...
	INSERT INTO [dbo].[CUS_YLOrdOriginalBOMData_OPP](ordId,pageName,itmID,itmIDInstance,olnID,olnIDInstance,itmIDParent,itmIDParentInstance,itmIDSA,itmDescriptionSA,dlCode,oriReqQty,primaryUOMCode,itmItemNumber,itmDescription,itmfCode,topSurCode,corCode,dimCX,dimCY,dimCZ,dimFX,dimFY,dimFZ,info14,info15)
	SELECT 
		@ordID AS ordID
		...
		,CAST(oDim.dimFY AS FLOAT) AS dimFY
		,CAST(oDim.dimFZ AS FLOAT) AS dimFZ
		,itmAv.ainfo1 AS info14
		,itmAv.info1 AS info15
	FROM T t
	JOIN T SA ON t.olnID=SA.olnID AND SA.dlCode=N'SA'
	JOIN dbo.ItemItemFamilies itmItemF (NOLOCK) ON itmItemF.itmID = t.itmID AND itmItemF.itmfCode =N'BFMKM_CM'
	OUTER APPLY
