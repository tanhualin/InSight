alter PROC [dbo].[spApp_GetYLOrdersNormal_BFMT_OPP]
	@ordID INT
	,@pageName NVARCHAR(50)=N'包覆门条'
...
BEGIN
	...
	JOIN dbo.ItemItemFamilies itmItemF (NOLOCK) ON itmItemF.itmID = t.itmID AND itmItemF.itmfCode =N'BFMKM_KT'
	OUTER APPLY
	(
		SELECT 
...
BEGIN
	...
	INSERT INTO [dbo].[CUS_YLOrdOriginalBOMData_OPP](ordId,pageName,itmID,itmIDInstance,olnID,olnIDInstance,itmIDParent,itmIDParentInstance,itmIDSA,itmDescriptionSA,dlCode,oriReqQty,primaryUOMCode,itmItemNumber,itmDescription,itmfCode,topSurCode,corCode,dimCX,dimCY,dimCZ,dimFX,dimFY,dimFZ)
	SELECT 
		@ordID AS ordID
