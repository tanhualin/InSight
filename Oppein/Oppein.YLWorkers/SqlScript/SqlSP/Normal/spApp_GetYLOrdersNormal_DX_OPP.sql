ALTER PROC [dbo].[spApp_GetYLOrdersNormal_DX_OPP]
	@ordID INT
	,@pageName NVARCHAR(10)=N'普通顶线'
	,@Factory NVARCHAR(50)=NULL
AS
SET NOCOUNT ON;

--DECLARE @ordID INT=2699608
--	,@pageName NVARCHAR(10)=N'普通顶线'
--	,@Factory NVARCHAR(50)
...
BEGIN
	...
	SELECT 
		...
		,REPLACE(imos.info1,N'边','') AS info1
		,ISNULL(org.info1,REPLACE(imos.info1,N'边','')) AS oInfo1
		,REPLACE(imos.info2,N'边','') AS info2
		,ISNULL(org.info2,REPLACE(imos.info2,N'边','')) AS oInfo2
		,coalesce(itmMat.itmDescription2, mr.matDescription) as info3
		,COALESCE(org.info3,itmMat.itmDescription2, mr.matDescription)AS oInfo3
		,org.actions
	...
	JOIN dbo.CustomSetting_Opp cs (NOLOCK) ON cs.FurnitureKittingList = 'DX' AND itmf.itmfCode=cs.SCode
	OUTER APPLY
	(
		SELECT 
	...
	OUTER APPLY(
		SELECT
			MAX(CASE WHEN oav.atbID = 115 THEN oav.itmavValue END) AS info1
			,MAX(CASE WHEN oav.atbID = 44 THEN oav.itmavValue END) AS info2
		FROM dbo.ItemAttributeValues oav (NOLOCK)
		WHERE oav.itmID = t.itmID
			AND oav.atbID IN (115,44)
	)imos
	JOIN dbo.OrderAttributeValues oav (NOLOCK) ON oav.atbID = 162
		AND oav.ordID = @ordID
...
BEGIN
	...
	INSERT INTO [dbo].[CUS_YLOrdOriginalBOMData_OPP](ordId,pageName,itmID,itmIDInstance,olnID,olnIDInstance,itmIDParent,itmIDParentInstance,itmIDSA,itmDescriptionSA,dlCode,oriReqQty,primaryUOMCode,itmItemNumber,itmDescription,itmfCode,itmfDescription,topSurCode,corCode
		,dimCX,dimCY,dimCZ,dimFX,dimFY,dimFZ,info1,info2,info3)
	SELECT 
		@ordID AS ordID
