...
ALTER PROC [dbo].[spApp_GetYLOrdersNormal_BL_OPP]
	@ordID INT
	,@pageName NVARCHAR(10)=N'普通玻璃'
	,@Factory NVARCHAR(50)=NULL
AS
SET NOCOUNT ON;

--DECLARE @ordID INT=2735001
--	,@pageName NVARCHAR(10)=N'普通玻璃'
--	,@Factory NVARCHAR(50)
...
BEGIN
	...
	SELECT 
		...
		,org.info15 AS info15	--工艺要求
		,ISNULL(ha.holeRemark,org.info15) AS oInfo15	--工艺要求
	...
	Join dbo.CustomSetting_Opp cs (NOLOCK) On cs.FurnitureKittingList=N'BL' and itmf.itmfCode=cs.SCode  
	OUTER APPLY
	(  
		SELECT 
...
BEGIN
	...
	SELECT 
		...
		,ha.Remark AS info6
		,ha.Remark AS oInfo6
		,ha.holeRemark AS info15	--工艺要求
		,ha.holeRemark AS oInfo15	--工艺要求
		,ISNULL(ha.Factory,@Factory) AS Factory
		,ha.Duty
		,ISNULL(ha.PlateCategory,@PlateCategory) AS PlateCategory
		,CAST(lp.LegacyPrice AS FLOAT) AS Price		--单价
