...
ALTER PROC [dbo].[spApp_GetYLOrdersNormal_GNJ_OPP]
	@ordID INT
	,@pageName NVARCHAR(10)=N'普通功能件'
...
BEGIN
	...
	--left join dbo.CustomizedItemFamily_Opp cz (NOLOCK) on cz.scode =itmf.itmfCode AND cz.ReportType=6    
	OUTER APPLY
	(  
		Select 
	...
	WHERE t.oriismfg = 1
		AND cs.FurnitureKittingList = N'GN'
		--AND (cs.FurnitureKittingList = N'GN' OR cz.ReportType=6 )
		AND mr.mtpCode NOT IN (N'Sheet Stock Finish',N'Sheet Stock Solid') --屏蔽实木，烤漆
