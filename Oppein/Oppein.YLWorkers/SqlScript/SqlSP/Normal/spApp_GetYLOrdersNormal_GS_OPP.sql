...
/*
	...
	EXEC [dbo].[spApp_GetYLOrdersNormal_GS_OPP] @ordID = 3962879 ,@pageName=N'普通柜身'

	DELETE FROM [dbo].[CUS_YLOrdOriginalBOMData_OPP]

	2、参数存储:spGetBackPanelCutListV2_OPP
	EXEC report.spGetCutListV5_OPP  @ordID = 4675210
*/

ALTER PROC [dbo].[spApp_GetYLOrdersNormal_GS_OPP]
	@ordID INT
	,@pageName NVARCHAR(10)=N'普通柜身'
	,@Factory NVARCHAR(50)=NULL
AS
SET NOCOUNT ON;

--DECLARE 	@ordID INT=3963044
--	,@pageName NVARCHAR(10)=N'普通柜身'
	--,@Factory NVARCHAR(50)
...
BEGIN
	...
	Join dbo.CustomSetting_Opp cs (NOLOCK) On cs.FurnitureKittingList IN(N'GS',N'LFB') and itmf.itmfCode=cs.SCode  
	OUTER APPLY
	(  
		SELECT 
	...
	WHERE t.oriIsPurchased = 0 
		AND t.itmpID = 2
		AND oDim.dimCX > 5
		AND mr.mtpCode NOT IN (N'Sheet Stock Finish',N'Sheet Stock Solid',N'Sheet Stock XS') --屏蔽实木，烤漆，吸塑
