...
BEGIN
	...
	SELECT 
		...
		,ISNULL(ha.WenLu,CASE WHEN iatbv.itmavValue = 0 AND t.itmDescription <> N'抽底' THEN N'竖纹' 
			  WHEN iatbv.itmavValue = 90 AND t.itmDescription <> N'抽底' THEN N'横纹' END) AS info5	--纹理
		,(CASE WHEN ISNUMERIC( f.info2)=1 AND CAST(f.info2 AS FLOAT) > 0 THEN f.info2 END)	AS info1							--距底孔1
		,(CASE WHEN f.info2!=f.info3 AND ISNUMERIC( f.info3)=1 AND CAST(f.info3 AS FLOAT) > 0  THEN f.info3 END)	AS info2		--距底孔1
		,COALESCE(org.info5,ha.WenLu,CASE WHEN iatbv.itmavValue = 0 AND t.itmDescription <> N'抽底' THEN N'竖纹' 
			  WHEN iatbv.itmavValue = 90 AND t.itmDescription <> N'抽底' THEN N'横纹' END)oInfo5
		,ISNULL(org.info2,CASE WHEN ISNUMERIC( f.info2)=1 AND CAST(f.info2 AS FLOAT) > 0 THEN f.info2 END) AS oInfo1
		,ISNULL(org.info7,CASE WHEN f.info2!=f.info3 AND ISNUMERIC( f.info3)=1 AND CAST(f.info3 AS FLOAT) > 0  THEN f.info3 END)AS oInfo2
		,org.actions
	...
	Join dbo.CustomSetting_Opp cs (NOLOCK) On cs.FurnitureKittingList=N'BB' and itmf.itmfCode=cs.SCode  
	OUTER APPLY
	(  
		Select 
	...
	WHERE  mr.mtpCode NOT IN (N'Sheet Stock Finish',N'Sheet Stock Solid',N'Sheet Stock XS') --屏蔽实木，烤漆，吸塑
	--ORDER BY cus.actions,itm.itmDescription
	ORDER BY itmf.itmfDescription,oDim.dimFX DESC,oDim.dimFY DESC,oDim.dimFZ DESC,t.itmID
END
ELSE
BEGIN
	;WITH T AS
	...
	SELECT
		...
		--,t.oriReqQty
		--,t.PrimaryUOMCode
		,org.itmID AS itmIDSA
		,org.itmDescription AS itmDescriptionSA
		,org.itmDescription
		...
		,org.info1
		,org.info1 AS oInfo1	--距底孔1
		,org.info2				--距底孔1
		,org.info2 AS oInfo2	--距底孔1
		,org.info5
		,org.info5 AS oInfo5	--纹理
		,org.actions
		,ISNULL(ha.Factory,@Factory) AS Factory
		,ha.Duty
		,ISNULL(ha.PlateCategory,@PlateCategory) AS PlateCategory
