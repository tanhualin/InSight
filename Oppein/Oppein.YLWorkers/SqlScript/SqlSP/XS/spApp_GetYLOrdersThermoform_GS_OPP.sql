ALTER PROC [dbo].[spApp_GetYLOrdersThermoform_GS_OPP]
	@ordID INT
	,@pageName NVARCHAR(10)=N'吸塑柜身'
	,@Factory NVARCHAR(50)=NULL
AS
SET NOCOUNT ON;

--DECLARE @ordID INT= 3304720
--	,@pageName NVARCHAR(10)=N'吸塑柜身'
--	,@Factory NVARCHAR(50)
...
BEGIN
	...
	SELECT 
		...
		,oItmgCNC.fxmPath2
		,org.actions
		,(CASE WHEN (ISNUMERIC(iav.X)=1 AND iav.X > 0) OR (ISNUMERIC(iav.Y)=1 AND iav.Y > 0)OR(ISNUMERIC(iav.Z)=1 AND iav.Z > 0)then N'对数控排孔' else ' ' end) info6
		,ISNULL(org.info6,(CASE WHEN (ISNUMERIC(iav.X)=1 AND iav.X > 0) OR (ISNUMERIC(iav.Y)=1 AND iav.Y > 0)OR(ISNUMERIC(iav.Z)=1 AND iav.Z > 0)then N'对数控排孔' else ' ' end)) AS oInfo6
	...
	Join dbo.CustomSetting_Opp cs (NOLOCK) On cs.FurnitureKittingList = N'GS' and itmf.itmfCode=cs.SCode  
	OUTER APPLY
	(  
		SELECT 
	...
	(
		SELECT 
			MAX(CASE WHEN oicnc.opmSequence=1 THEN (CASE WHEN CHARINDEX(N'/',oicnc.oriCNCProgOutputPath)>0 THEN oicnc.oriCNCProgOutputPath+oicnc.oriCNCProgName+N'.'+oicnc.oriCNCProgExtension ELSE oicnc.oriCNCProgOutputPath+N'\'+oicnc.oriCNCProgName+N'.'+oicnc.oriCNCProgExtension END) END) AS fxmPath1
			,MAX(CASE WHEN oicnc.opmSequence=2 THEN (CASE WHEN CHARINDEX(N'/',oicnc.oriCNCProgOutputPath)>0 THEN oicnc.oriCNCProgOutputPath+oicnc.oriCNCProgName+N'.'+oicnc.oriCNCProgExtension ELSE oicnc.oriCNCProgOutputPath+N'\'+oicnc.oriCNCProgName+N'.'+oicnc.oriCNCProgExtension END) END) AS fxmPath2
		FROM dbo.OrderItemCNCPrograms oicnc
		WHERE oicnc.itmID=t.itmID
			AND oicnc.itmIDInstance=t.itmIDInstance
			AND oicnc.olnID=t.olnID
			AND oicnc.olnIDInstance=t.olnIDInstance
			AND oicnc.oprID=229
	)oItmgCNC
	...
	WHERE mr.mtpCode = N'Sheet Stock XS'
...
BEGIN
	...
	INSERT INTO [dbo].[CUS_YLOrdOriginalBOMData_OPP](ordId,pageName,itmID,itmIDInstance,olnID,olnIDInstance,itmIDParent,itmIDParentInstance,itmIDSA,itmDescriptionSA,dlCode,oriReqQty,primaryUOMCode,itmItemNumber,itmDescription,itmfCode,itmfDescription,topSurCode,corCode,edgeCode,dimCX,dimCY,dimCZ,dimFX,dimFY,dimFZ,info6)
	SELECT 
		@ordID AS ordID
	...
	Join dbo.CustomSetting_Opp cs (NOLOCK) On cs.FurnitureKittingList = 'GS' and itmf.itmfCode=cs.SCode  
	OUTER APPLY
	(  
		SELECT 
			MAX(Case when orie.orieEdgeNo=4 then IsNull(iatbv.itmavValue,0) end) as Edge1  
	...
	(
		SELECT 
			...
			,MAX(CASE WHEN oicnc.opmSequence=2 THEN (CASE WHEN CHARINDEX(N'/',oicnc.oriCNCProgOutputPath)>0 THEN oicnc.oriCNCProgOutputPath+oicnc.oriCNCProgName+N'.'+oicnc.oriCNCProgExtension ELSE oicnc.oriCNCProgOutputPath+N'\'+oicnc.oriCNCProgName+N'.'+oicnc.oriCNCProgExtension END) END) AS fxmPath2
		FROM dbo.OrderItemCNCPrograms oicnc
		WHERE oicnc.itmID=org.itmID
			AND oicnc.itmIDInstance=org.itmIDInstance
			AND oicnc.olnID=org.olnID
			AND oicnc.olnIDInstance=org.olnIDInstance
			AND oicnc.oprID=229
	)oItmgCNC
