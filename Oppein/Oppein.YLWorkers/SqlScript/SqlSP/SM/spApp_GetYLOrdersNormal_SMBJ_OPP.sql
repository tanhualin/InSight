ALTER PROC [dbo].[spApp_GetYLOrdersNormal_SMBJ_OPP]
	@ordID INT
	,@pageName NVARCHAR(10)=N'实木部件'
...
BEGIN
	...
	(
		...
		JOIN dbo.OrderAttributeValues ordAv (NOLOCK)on ordAv.ordID=ord.ordID and ordAv.atbID=162
	...
	SELECT 
		...
		,orim.topSurCode 
		,orim.corCode
		,org.info15 AS info16	--路轨(CAD)
		,COALESCE(ha.Remark,org.info6) AS info6
		,COALESCE(org.info6,ha.Remark) AS oInfo6
	...
	JOIN dbo.CustomSetting_Opp cs on cs.SCode = itmf.itmfCode 
		...
		AND itmf.itmfCode IN( N'ZSBJ-SM',N'ZSMB-SM',N'ZSCB-SM')
	OUTER APPLY
	(
		SELECT 
	...
	OUTER APPLY(
		...
		from dbo.OrderItems ori_pp (NOLOCK)
		join dbo.ItemItemFamilies itmif (NOLOCK)on itmif.itmID=ori_pp.itmID and itmif.itmfCode='Railway'
		join dbo.Items itm_pp (NOLOCK)on itm_pp.itmID=ori_pp.itmID
...
BEGIN
	...
	SELECT 
		...
		,ISNULL(itmpp_CAD.Railway,itmAv.info16) AS info15	--路轨(CAD)
		
	FROM T t
	JOIN T SA ON t.olnID=SA.olnID AND SA.dlCode=N'SA'
	JOIN dbo.ItemItemFamilies itmItemF (NOLOCK) ON itmItemF.itmID = t.itmID AND itmItemF.itmfCode IN( N'ZSBJ-SM',N'ZSMB-SM',N'ZSCB-SM')
	JOIN dbo.ItemFamilies itmf (NOLOCK) ON itmf.itmfCode = itmItemF.itmfCode
	JOIN dbo.CustomSetting_Opp cs on cs.SCode = itmf.itmfCode 
		AND cs.SDesc=N'SMM'	--IN ( 'SMM','SMC','SMD','SMZ') 
	LEFT JOIN dbo.OrderItemMaterials orim(NOLOCK) ON orim.itmID = t.itmID 
	...
	outer apply      
	(      
		SELECT (CASE WHEN iav.atbID=44 THEN 
				(CASE WHEN iav.itmavValue='1' THEN N'三节路轨10"(海蒂斯)'   
				  WHEN iav.itmavValue='2' THEN N'三节路轨16"(海蒂斯)'  
				  WHEN iav.itmavValue='3' THEN N'三节路轨14"(海蒂斯)'      
				  WHEN iav.itmavValue='4' THEN N'三节路轨16"(海蒂斯)'  
