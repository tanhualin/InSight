ALTER PROC [dbo].[spApp_GetYLOrdersNormalCAD_SMWJ_OPP]
	@ordID INT
...
BEGIN
	...
	(
		...
		SELECT
			...
			,t.oriReqQty AS oriReqQty
			,t.PrimaryUOMCode
			,t.itmDescription AS itmItemNumber
			,REPLACE(t.itmDescription2,N'五金','') AS itmDescription
			,t.itmfCode
			,id.dimX as dimFX
			,id.dimY as dimFY
			,id.dimZ as dimFZ
			,N'条' AS UOMDescription
		FROM TT t
		LEFT JOIN dbo.ItemDimensions id (NOLOCK) ON id.dimID = 1 AND id.itmID = t.itmID
		WHERE t.dlCode = 'PP'
			AND NOT EXISTS(SELECT 1 FROM TT tt JOIN dbo.CustomSetting_Opp cs (NOLOCK) ON CS.FurnitureKittingList ='SM' AND cs.SDesc = 'SMH' AND tt.itmfCode=cs.SCode 
				WHERE tt.itmID=t.itmID AND tt.itmIDInstance=t.itmIDInstance AND tt.olnID=t.olnID AND tt.olnIDInstance=t.olnIDInstance)
			AND  t.itmItemNumber LIKE N'实木五金%'
	),TS AS
	(
		SELECT
			t.itmID
...
BEGIN
	...
	LEFT JOIN dbo.ItemDimensions id (NOLOCK) ON id.dimID = 1 AND id.itmID = t.itmID
	WHERE t.dlCode = 'PP'
		AND NOT EXISTS(SELECT 1 FROM T tt JOIN dbo.CustomSetting_Opp cs (NOLOCK) ON CS.FurnitureKittingList ='SM' AND cs.SDesc = 'SMH' AND tt.itmfCode=cs.SCode 
			WHERE tt.itmID=t.itmID AND tt.itmIDInstance=t.itmIDInstance AND tt.olnID=t.olnID AND tt.olnIDInstance=t.olnIDInstance)
		AND  t.itmItemNumber LIKE N'实木五金%'
