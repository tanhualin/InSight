--EXEC [dbo].[spApp_GetYLOrdersNormal_KQM_OPP] @ordID= 6650642,@pageName =N'烤漆门抽面'
--EXEC [dbo].[spApp_GetManualOrdersNormal_KQM_OPP] @ordID= 6650642,@pageName =N'烤漆门抽面'


alter PROC [dbo].[spApp_GetYLOrdersNormal_KQM_OPP]
	@ordID INT
	,@pageName NVARCHAR(10)=N'烤漆门抽面'
...
BEGIN
	...
	JOIN dbo.CustomSetting_Opp cs ON cs.SCode = itmf.itmfCode and cs.SDesc=N'KQM' AND itmf.itmfCode=N'DOOR-KQ'
	OUTER APPLY
	(
		SELECT 
...
BEGIN
	...
	(
		SELECT 
			...
			,itm.itmDescription
			,itm.PrimaryUOMCode
			,oFtr.UT48_127 AS UT48_127	--装饰部件的备注
		FROM dbo.Orders ord (NOLOCK)
		JOIN dbo.OrderLines oln (NOLOCK) ON oln.ordID = ord.ordID
		OUTER APPLY
		(
